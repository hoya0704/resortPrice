...
//echo $url_segment;
		...
		//페이지네이션 설정
		$config['base_url'] = LOCAL_PATH.'/index.php/Reservation/lists/RESERVATION/' .$page_url .'/page/';		//페이징 주소
		$config['total_rows'] = $this->reservation_m->get_list($this->uri->segment(3), 'count', '', '', $search_word);		//게시물의 전체개수
...
//echo $search_word ; 
		...
		$data['list'] =$this->reservation_m->get_list($this->uri->segment(3), '', $start, $limit, $search_word);
		//print_r ($data);
		$this->load->view('reservation/list_v', $data);
	...
	*/
	function view()
	{
		$data['nation'] =$this->common_m->get_common_code_list(4);			//국가 샐렉트박스		
		//게시판 이름과 게시물 번호에 해당하는 게시물 가져오기
		$data['views'] = $this->reservation_m->get_view($this->uri->segment(3), $this->uri->segment(5));
		//view 호출
		$this->load->view('reservation/view_v', $data);
	}
...
/**/
			...
			//쓰기 폼 view 호출
			$this -> load -> view('reservation/write_v', $data);
			//$this -> load -> view('reservation/write_v');
		}
	}
